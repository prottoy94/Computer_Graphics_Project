...
// ── Summer Mode Dynamic Transformation State ─────────────────────
//
//  ROTATION   : coconut-stall ceiling fan
//               fanAngle auto-increments every tick via glRotatef().
//               '1' slows the fan, '2' speeds it up.
//
//  SCALING    : beach umbrella
//               umbrellaScale applied via glScalef() when drawing.
//               '3' shrinks, '4' enlarges.
//
//  TRANSLATION: ice cream cart rolls across the footpath.
//               cartX advances each tick; speed via '5'(slower)/'6'(faster).
//
//  SELECTION  : clicking the sun with the mouse toggles sunSelected.
//               A bright ring is drawn around the sun when selected.
//
//  PROJECTION : an elliptical shadow is projected beneath the beach
//               umbrella onto the ground, scaled by umbrella size.

static float fanAngle      = 0.0f;   // current fan rotation (deg)
static float fanSpeed      = 3.5f;   // degrees per tick
static float umbrellaScale = 1.0f;   // beach umbrella size multiplier
static float cartX         = -0.95f; // ice cream cart X position
static float cartSpeed     = 0.0030f;// cart translation speed per tick
static bool  sunSelected   = false;  // true when user has clicked the sun
static float kiteAngle     = 0.0f;   // kite gentle bob phase
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    // ── Summer: Rotation — fan speed (keys '1' / '2') ────────────────────
    case '1': // slow fan
        if (summerMode) { fanSpeed -= 1.0f; if (fanSpeed < 0.0f) fanSpeed = 0.0f; }
        break;
    case '2': // speed up fan
        if (summerMode) { fanSpeed += 1.0f; if (fanSpeed > 15.0f) fanSpeed = 15.0f; }
        break;

    // ── Summer: Scaling — beach umbrella size (keys '3' / '4') ────────────
    case '3': // shrink umbrella
        if (summerMode) { umbrellaScale -= 0.15f; if (umbrellaScale < 0.3f) umbrellaScale = 0.3f; }
        break;
    case '4': // grow umbrella
        if (summerMode) { umbrellaScale += 0.15f; if (umbrellaScale > 3.0f) umbrellaScale = 3.0f; }
        break;

    // ── Summer: Translation — ice cream cart speed (keys '5' / '6') ─────
    case '5': // slow cart
        if (summerMode) { cartSpeed -= 0.001f; if (cartSpeed < 0.0f) cartSpeed = 0.0f; }
        break;
    case '6': // speed up cart
        if (summerMode) { cartSpeed += 0.001f; if (cartSpeed > 0.015f) cartSpeed = 0.015f; }
        break;
...
// ================================================================
//  SUMMER — ROTATING CEILING FAN on the coconut stall  [Summer/Rotation]
//
//  The fan is centred on top of the stall canopy pole.
//  fanAngle is incremented every tick; glRotatef() spins the blades.
//  Keys '1' (slower) and '2' (faster) adjust fanSpeed.
// ================================================================
static void drawSummerFan()
{
    if (!summerMode) return;
    float cx = -0.28f, cy = -0.615f; // centre above stall canopy
    float bladeLen = 0.038f;

    glPushMatrix();
    glTranslatef(cx, cy, 0.0f);
    glRotatef(fanAngle, 0.0f, 0.0f, 1.0f); // ROTATION via glRotatef

    // 4 blades
    for (int b = 0; b < 4; b++)
    {
        float a0 = b * 90.0f * PI / 180.0f;
        float a1 = a0 + 0.55f;
        glColor4ub(240, 220, 80, 210);
        glBegin(GL_TRIANGLES);
        glVertex2f(0.0f, 0.0f);
        glVertex2f(cosf(a0) * bladeLen, sinf(a0) * bladeLen);
        glVertex2f(cosf(a1) * bladeLen, sinf(a1) * bladeLen);
        glEnd();
    }
    // Hub
    diskFan(0.0f, 0.0f, 0.007f, 0.007f, 10, 180, 60, 30);
    glPopMatrix();
}

// ================================================================
//  SUMMER — BEACH UMBRELLA  [Summer/Scaling + Projection]
//
//  Drawn with glScalef(umbrellaScale, umbrellaScale, 1) so keys
//  '3'/'4' visibly shrink/grow it.
//  An elliptical shadow (projection) is drawn beneath it on the ground.
// ================================================================
static void drawBeachUmbrella()
{
    if (!summerMode) return;
    float bx = 0.72f, by = -0.715f; // position on footpath right side

    // ── Shadow projection onto the ground strip ──
    // Shadow is always drawn (unscaled) so it remains anchored to ground.
    glColor4ub(80, 50, 10, 60);
    glBegin(GL_TRIANGLE_FAN);
    glVertex2f(bx, by - 0.003f);
    for (int k = 0; k <= 18; k++)
    {
        float a = k * 2.f * PI / 18.f;
        glVertex2f(bx + cosf(a) * 0.06f * umbrellaScale,
                   by - 0.003f + sinf(a) * 0.010f);
    }
    glEnd();

    // ── Umbrella body (SCALING via glScalef) ──
    glPushMatrix();
    glTranslatef(bx, by, 0.0f);
    glScalef(umbrellaScale, umbrellaScale, 1.0f); // SCALING

    // Pole
    solidQ(-0.004f, 0.0f, 0.004f, 0.12f, 180, 160, 120);

    // Canopy — 6 alternating stripes (red / white)
    for (int s = 0; s < 6; s++)
    {
        float a1 = s * 2.f * PI / 6.f;
        float a2 = (s + 1) * 2.f * PI / 6.f;
        unsigned char r = (s % 2 == 0) ? 220 : 255;
        unsigned char g = (s % 2 == 0) ? 40  : 255;
        unsigned char b = (s % 2 == 0) ? 40  : 255;
        glColor3ub(r, g, b);
        glBegin(GL_TRIANGLES);
        glVertex2f(0.0f, 0.12f);
        glVertex2f(cosf(a1) * 0.09f, 0.12f + sinf(a1) * 0.035f);
        glVertex2f(cosf(a2) * 0.09f, 0.12f + sinf(a2) * 0.035f);
        glEnd();
    }
    // Canopy rim
    ...
    {
        float a = k * 2.f * PI / 18.f;
        glVertex2f(cosf(a) * 0.09f, 0.12f + sinf(a) * 0.035f);
    }
    glEnd();
    glLineWidth(1.0f);
    glPopMatrix();
...
// ================================================================
//  SUMMER — ICE CREAM CART  [Summer/Dynamic Translation]
//
//  cartX advances each tick in updateSummer(); the cart is drawn
//  via glTranslatef(cartX, …) so the motion is clearly a
//  dynamic translation.  Keys '5'/'6' change cartSpeed.
// ================================================================
static void drawIceCreamCart()
{
    if (!summerMode) return;

    glPushMatrix();
    glTranslatef(cartX, -0.715f, 0.0f); // DYNAMIC TRANSLATION

    // Cart body (white box)
    solidQ(-0.040f, 0.0f, 0.040f, 0.055f, 245, 245, 255);
    // Blue stripe
    solidQ(-0.040f, 0.042f, 0.040f, 0.055f, 60, 100, 220);
    // Ice cream cone sign on side
    glColor3ub(255, 180, 50);
    glBegin(GL_TRIANGLES);
    glVertex2f(-0.010f, 0.020f);
    glVertex2f(0.010f, 0.020f);
    glVertex2f(0.000f, 0.005f);
    glEnd();
    diskFan(0.0f, 0.025f, 0.010f, 0.010f, 10, 255, 120, 160);
    // Handle / push bar
    solidQ(0.035f, 0.030f, 0.048f, 0.033f, 140, 100, 60);
    solidQ(0.046f, 0.010f, 0.050f, 0.033f, 140, 100, 60);
    // Two wheels
    diskFan(-0.025f, -0.006f, 0.010f, 0.010f, 12, 60, 60, 70);
    diskFan( 0.025f, -0.006f, 0.010f, 0.010f, 12, 60, 60, 70);
    diskFan(-0.025f, -0.006f, 0.004f, 0.004f, 8, 200, 200, 210);
    diskFan( 0.025f, -0.006f, 0.004f, 0.004f, 8, 200, 200, 210);
...
// ================================================================
//  SUMMER — KITE in the sky  [new summer component]
//
//  A classic diamond kite with a tail, bobbing gently using
//  a sine-wave offset on Y. The string is drawn from the kite
//  down to the footpath area.
// ================================================================
static void drawSummerKite()
{
    if (!summerMode) return;

    float kx = 0.55f;
    float ky = 0.62f + sinf(kiteAngle) * 0.03f; // gentle vertical bob

    // String (from kite down to footpath)
    glColor4ub(100, 80, 40, 180);
    glLineWidth(0.8f);
    glBegin(GL_LINE_STRIP);
    for (int i = 0; i <= 14; i++)
    {
        float t = (float)i / 14.f;
        float sx = kx + t * (0.68f - kx);
        float sy = ky + t * (-0.70f - ky);
        float sag = -0.06f * sinf(t * PI); // catenary sag
        glVertex2f(sx, sy + sag);
    }
    glEnd();

    // Kite diamond body — 4 coloured triangles
    float hw = 0.038f, hh = 0.055f;
    glColor3ub(230, 60, 60);
    glBegin(GL_TRIANGLES);
    glVertex2f(kx,       ky + hh);
    glVertex2f(kx - hw,  ky);
    glVertex2f(kx + hw,  ky);
    ...
    glLineWidth(1.0f);
    glBegin(GL_LINES);
    glVertex2f(kx, ky + hh); glVertex2f(kx, ky - hh);
    glVertex2f(kx - hw, ky); glVertex2f(kx + hw, ky);
    glEnd();
    // Tail ribbons
    glColor4ub(255, 120, 40, 200);
    glLineWidth(1.4f);
    glBegin(GL_LINE_STRIP);
    for (int i = 0; i <= 12; i++)
    {
        float t = (float)i / 12.f;
        glVertex2f(kx + sinf(t * PI * 3.f + kiteAngle) * 0.018f,
                   ky - hh - t * 0.10f);
    }
    glEnd();
    glLineWidth(1.0f);
...
// ================================================================
//  SUMMER — SUN SELECTION RING  [Selection]
//
//  When sunSelected == true (toggled by mouse click near the sun),
//  a bright pulsing ring is drawn around the sun using glRotatef
//  to spin dashed tick marks — demonstrating selection highlight.
// ================================================================
static void drawSunSelectionRing(float sx, float sy)
{
    ...
    pulse += 0.06f;
    float r = 0.115f + sinf(pulse) * 0.008f;

    glColor4f(1.0f, 0.95f, 0.3f, 0.85f);
    glLineWidth(2.2f);
    glPushMatrix();
    glTranslatef(sx, sy, 0.0f);
    glRotatef(pulse * 30.0f, 0.0f, 0.0f, 1.0f); // ROTATION for selection ring
    // Dashed ring (every other segment drawn)
    int segs = 32;
    for (int i = 0; i < segs; i += 2)
    {
        float a1 = i       * 2.f * PI / segs;
        float a2 = (i + 1) * 2.f * PI / segs;
        glBegin(GL_LINES);
        glVertex2f(cosf(a1) * r, sinf(a1) * r);
        glVertex2f(cosf(a2) * r, sinf(a2) * r);
        glEnd();
    }
    glPopMatrix();
    glLineWidth(1.0f);

    // Small label "SELECTED" near the sun
    glColor4f(1.0f, 1.0f, 0.2f, 0.90f);
    glRasterPos2f(sx + 0.13f, sy + 0.04f);
    const char* lbl = "[SUN SELECTED]";
    for (int c = 0; lbl[c]; c++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_10, lbl[c]);
}
...
void display()
{
    ...
    else if (summerMode)
    {
        ...
        drawStars();
        {
            ...
            getSunPos(_sx, _sy);
            drawSunAt(_sx, _sy);
            drawSunSelectionRing(_sx, _sy); // SELECTION: ring when sun clicked
        }
        drawMoon();
        drawSummerKite();   // new: kite in sky
        drawSummerGround();
    }
    ...
    if (summerMode)
    {
        drawSummerTint();
        drawSummerHeatHaze();
        // Original summer decorations
        drawCoconutStall();
        drawKidsEatingMango();
        drawBirdsResting();
        drawDryGroundPatches();
        drawWavyHeatHaze();
        // New summer components with transformations
        drawSummerFan();     // ROTATION   : spinning fan on stall (keys 1/2)
        drawBeachUmbrella(); // SCALING     : beach umbrella + shadow projection (keys 3/4)
        drawIceCreamCart();  // TRANSLATION : rolling ice cream cart (keys 5/6)
    }
...
void update(int value)
{
    ...
    // ── Summer — animate fan (rotation), cart (translation), kite bob ──
    if (summerMode)
    {
        fanAngle += fanSpeed;           // ROTATION  : fan blades spin
        if (fanAngle >= 360.0f) fanAngle -= 360.0f;

        cartX += cartSpeed;             // TRANSLATION : cart rolls right
        if (cartX > 1.10f) cartX = -1.10f; // wrap around

        kiteAngle += 0.04f;             // kite gentle bob
        if (kiteAngle > 2.0f * PI) kiteAngle -= 2.0f * PI;
    }
...
// ================================================================
//  MOUSE CALLBACK — Sun Selection  [Summer/Selection]
//
//  When the user left-clicks near the sun in summer mode, the sun
//  is toggled selected/deselected.  The window (X,Y) pixel coords
//  are mapped back to the world [-1,+1] space, then a distance
//  test against the sun position determines a hit.
// ================================================================
void mouse(int button, int state, int px, int py)
{
    ...
    // Map pixel to world coords (OpenGL Y is flipped)
    float wx = ((float)px / W) * 2.0f - 1.0f;
    float wy = 1.0f - ((float)py / H) * 2.0f;
...
int main(int argc, char **argv)
{
    ...
    glutMouseFunc(mouse); // SELECTION: click the sun to select it
    init();
